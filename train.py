...
def train(opt,train_loader,netG,netD):
    ...
    for epoch in range(0, n_epochs):
        ...
        for i in range(0,len(train_loader)):
            ...
            state = default_evaluator.run([[gen_targ, target]])
            #print("SSIM- ", state.metrics['ssim'])
            avg_ssim += state.metrics['ssim']

            #print("gen target shape-", gen_targ.shape)
            #padded_gen_targ = F.pad(gen_targ, (86,86,86,86), "constant", 0)
            padded_gen_targ = F.interpolate(gen_targ, size=(299, 299), mode='bicubic', align_corners=False)
            #print("padded gen target shape-", padded_gen_targ.shape)
            state1 = default_evaluator1.run([padded_gen_targ])
            avg_is += state1.metrics["is"]
            #print("Inseption Score: ", state1.metrics["is"])
            ...
            if (i + 1) % 100 == 0:
                ...
                print("Average SSIM: ", avg_ssim / (i+1))
                print("Average Inception Score: ", avg_is / (i+1))
    ...
    #             os.mkdir(save_dir)
        ...
        lr_scheduler_G.step()
        lr_scheduler_D.step()
        avg_ssim = avg_ssim / len(train_loader)
        print("Average SSIM for Epoch ", epoch, ": ", avg_ssim)
        avg_ssim_list.append(avg_ssim)
    epoch_list = list(np.arange(n_epochs))
    plt.plot(epoch_list, avg_ssim_list)
    plt.xlabel('Epochs')
    plt.ylabel('Average SSIM')
    plt.title('Epochs vs Average SSIM- Our Model')
    plt.savefig("ssim_results.png")
    plt.close()
